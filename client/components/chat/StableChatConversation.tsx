...
interface StableChatConversationProps {
  user: User;
...
export default function StableChatConversation({
  user,
  conversation,
  onBack,
  onConversationUpdate,
}: StableChatConversationProps) {
  ...
  const normalizeMessages = (rawMessages: any[]): Message[] => {
    return rawMessages.map((msg) => ({
      ...msg,
      content: msg.content || (msg as any).message || (msg as any).text || "",
  ...
  const loadMessages = async () => {
    try {
      ...
      if (isOnline) {
        ...
        chatStorage.saveMessages(conversation.user.id, enhancedMessages as any);
      ...
      console.error("خطأ في ��حميل الرسائل:", error);
  ...
  const sendMessage = async () => {
    ...
    chatStorage.addMessage(conversation.user.id, tempMessage as any);
    ...
    if (isOnline) {
      try {
        ...
        chatStorage.addMessage(conversation.user.id, sentMessage as any);
        ...
        chatStorage.addMessage(conversation.user.id, failedMessage as any);
      } finally {
        setIsSending(false);
      }
    }
    ...
    for (const { conversationId, message } of pendingMessages) {
      if (conversationId === conversation.user.id) {
        try {
          ...
          chatStorage.addMessage(conversationId, sentMessage as any);
        } catch (error) {
  ...
  const getDeliveryStatusIcon = (message: Message) => {
    ...
    switch (message.delivery_status) {
      case "sending":
        return (
          <Clock className={cn(iconClasses, "text-slate-400 animate-pulse")} />
        );
      case "failed":
        return <AlertCircle className={cn(iconClasses, "text-red-400")} />;
      case "sent":
        return <Check className={cn(iconClasses, "text-slate-400")} />;
      case "delivered":
        return <CheckCheck className={cn(iconClasses, "text-slate-400")} />;
      case "read":
        return <CheckCheck className={cn(iconClasses, "text-blue-400")} />;
      default:
        return <Check className={cn(iconClasses, "text-slate-400")} />;
    }
  };
  ...
  return (
    <div className="h-full bg-slate-900 dark:bg-slate-950 text-white flex flex-col relative overflow-hidden">
      {/* مؤشر حالة الاتصال */}
      {!isOnline && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <div
                    className={cn(
                      "absolute bottom-0 right-0 w-4 h-4 border-2 border-slate-800 rounded-full",
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 hover:bg-slate-700 text-slate-300 hover:text-white rounded-full"
              >
              ...
              {messageGroups.map(({ date, messages: groupMessages }) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-center my-6">
                    <div className="bg-slate-800/80 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-700">
                      ...
                      >
                        ...
                        >
                          ...
                          {isReply && repliedMessage && (
                            <div
                              className={cn(
                                "mb-1 p-2 rounded-lg bg-slate-800 border-l-2 text-xs max-w-[200px]",
                                isOwn
                                  ? "border-blue-500 mr-2"
                                  : "border-slate-500 ml-2",
                              )}
                            >
                          ...
                          <div
                            className={cn(
                              ...
                              isOwn
                                ...
                                : "bg-slate-800 text-white rounded-bl-lg border border-slate-700",
                            )}
                            onClick={() =>
                              setSelectedMessage(
                            ...
                            <AnimatePresence>
                              {selectedMessage === message.id && (
                                <motion.div
                                  ...
                                  className={cn(
                                    "absolute top-0 flex gap-1 p-1 bg-slate-800/95 rounded-lg border border-slate-700 shadow-xl",
                                    isOwn
                                ...
                                >
                                  ...
                                  {isOwn && (
                                    <Button
                                      variant="ghost"
                                      size="icon"
                                      className="h-6 w-6 hover:bg-red-900/20 text-red-400"
                                    >
      ...
      <AnimatePresence>
        {replyToMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="relative z-30 px-4 py-2 bg-slate-800 border-t border-slate-700"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm truncate max-w-[200px] text-white">
      ...
      <AnimatePresence>
        {showAttachments && (
          <motion.div
            ...
            className="relative z-30 px-4 py-3 bg-slate-800 border-t border-slate-700"
          >
            <div className="flex items-center gap-4 justify-center">
              <Button
                variant="ghost"
                size="sm"
                className="flex flex-col gap-1 h-auto p-3 hover:bg-slate-700 rounded-2xl"
              >
      ...
      {/* منطقة الكتابة الثابتة والمحسنة */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-800/95 backdrop-blur-xl border-t border-slate-700">
        <div className="px-4 py-3">
          <div className="flex items-end gap-3">
            ...
            <div className="flex-1 relative">
              <div className="bg-slate-700/50 rounded-3xl border border-slate-600 transition-all duration-200 focus-within:border-blue-500 focus-within:bg-slate-700/70 focus-within:shadow-lg">
                <div className="flex items-end">
                  <div className="flex-1 px-4 py-3">
                    <textarea
                      ...
                      rows={1}
                      className="w-full resize-none bg-transparent text-sm text-right placeholder:text-slate-400 focus:outline-none leading-5 scrollbar-hide text-white"
                      style={{
                        fontFamily: "inherit",
                        lineHeight: "20px",
                      }}
                  ...
                  <div className="flex items-center gap-1 px-2 pb-3">
                    <div className="relative">
                      <Button
                        ...
                        className="h-8 w-8 rounded-full hover:bg-slate-600 text-slate-400 hover:text-white"
                      >
                      ...
                      <AnimatePresence>
                        {showEmojiPicker && (
                          <motion.div
                            ...
                            className="absolute bottom-full right-0 mb-2 z-50 bg-slate-800 border border-slate-600 rounded-lg p-4 shadow-xl"
                          >
        ...
        {/* مساحة آمنة للكيبورد */}
        <div className="h-[env(keyboard-inset-height)] bg-slate-800/95 backdrop-blur-xl"></div>
      </div>
