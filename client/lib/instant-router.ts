...
class InstantRouter {
  ...
  registerRoute(config: RouteConfig): void {
    ...
    // Cache the component immediately
  ...
  private async navigateInternal(
    ...
    try {
      // Preload the route if not cached
  ...
  async preloadRoute(path: string): Promise<void> {
    ...
    try {
      // Component is already cached during registration
...
// React hooks for using the router
...
export function useNavigationGuard(
  ...
  React.useEffect(() => {
    // This would be implemented if we need navigation guards
    // For now, it's a placeholder for future enhancement
  }, [guard, navigationState]);
}

// Preloader hook for route optimization
